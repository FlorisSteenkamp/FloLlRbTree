...
describe('remove', function() {
    ...
    function() {
        {
            ...
            expect(() => tree.remove({ val: 112 })).not.toThrow();

            insert1to7Objs(tree);
            tree.remove({ val: 2 });
            expect(tree.toArrayInOrder().map(o => o.val)).toEqual([1,3,3,4,5,6,7]);
            tree.remove({ val: 5, name: 'apple pear' });
            expect(tree.toArrayInOrder().map(o => o.val)).toEqual([1,3,3,4,6,7]);
            ...
            expect(tree.toArrayInOrder().map(o => o.val)).toEqual([7]);
    ...
    function() {
        ...
        expect(vals).toEqual([1,1,2,2,3,3,3,3.5,4,4,5,5,6,6,7,7]);
        
        tree.remove({ val: 3 }, true);
        // treeToString(tree, nodeObjToString);
        vals = tree.toArrayInOrder().map(o => o.val);
        expect(vals).toEqual([1,1,2,2,3.5,4,4,5,5,6,6,7,7]);
    });
